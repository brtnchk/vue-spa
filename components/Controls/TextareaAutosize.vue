  ...
  export default {
    ...
    methods: {
      ...
      resize: function () {
        ...
        let stayDefaultSize = (this.$el.clientWidth <= 55)
          ? (this.$el.value.length < 1)
          : this.$el.value.length < (Math.ceil((this.$el.clientWidth - 55)/8));
        ...
        const heightVal = stayDefaultSize ? '55px' : contentHeight + 'px';
        this.$el.style.setProperty('height', heightVal, important);
        this.$emit('resize', heightVal);
        return this;
      }
    },
    watch: {
